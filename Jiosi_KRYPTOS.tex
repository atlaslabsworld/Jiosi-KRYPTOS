...
\begin{abstract}
\noindent Kryptos, Jim Sanborn's cryptographic sculpture at CIA headquarters, contains three publicly solved sections and a fourth section, \Kfour, whose complete plaintext and method remain unpublished. Public constraints identify two \Kfour\ plaintext spans: \texttt{EASTNORTHEAST} and \texttt{BERLINCLOCK}. Sanborn's 2025 remarks add that a future \Kfive\ message will be 97 characters long, will involve elements of the earlier puzzles, and will share some coded words with \Kfour\ in the same positions \cite{verganoFinalClues2025}.

This paper introduces \textbf{Paired Research Optimization} (PRO): a constraint-first procedure for generating, ranking, and falsifying paired \Kfour/\Kfive\ plaintext hypotheses without claiming a decryption. I treat \Kfour\ as a 97-character normalized plaintext search anchored by public cribs and \Kfive\ as a companion constrained by public meta-information. PRO combines length checks, fixed crib satisfaction, modular residue checks, same-position word-skeleton alignment, thematic evidence from Kryptos and Sanborn's public clues, cultural and geographic robustness, and a penalty for confusing plausible text with a demonstrated cipher mechanism.

The contribution is a reproducible search frame, not a solved plaintext. It separates plaintext plausibility, public constraint satisfaction, and independent cipher-method recovery. A top-ranked candidate pair is presented with its interval structure, scoring rationale, Berlin-clock interpretation, and failure conditions. More generally, PRO is a constrained optimization framework that performs a best-guess, brute-force search over candidate \Kfour/\Kfive\ plaintext pairs $(P_4,Q_5)$, ranking hypotheses according to publicly known positional, thematic, linguistic, geographic, and modular constraints while explicitly distinguishing plaintext plausibility from cryptanalytic validity.
\end{abstract}
\end{frontmatter}
...
This appendix records the mechanical checks used to verify the candidate table and the modular arithmetic claims. The audit code is included inline in \autoref{code:appendixAuditScript}; executing the listing in a Python interpreter prints \texttt{Appendix A audit passed} when all checks below hold.
...
The audit uses two indexing conventions, and the distinction is intentional. Python slices such as \texttt{n4[21:34]} are zero-based and half-open, so they test the paper's one-based interval 22--34. By contrast, the word-interval routine initializes \texttt{position = 1} because the intervals reported in \autoref{tab:sharedIntervals} and in the set notation are published character positions over $1,\ldots,97$, not Python indexes. Thus \texttt{MESSAGE} in the top pair is reported as positions 10--16 rather than 9--15.

\begin{lstlisting}[style=paperPython,caption={Python audit for Appendix A reproducibility checks.},label={code:appendixAuditScript}]
import re
